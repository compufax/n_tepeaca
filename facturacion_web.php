...
if($_POST['ajax']==1){
	$Root = mysql_fetch_assoc(mysql_query("SELECT facturarsinentrega FROM usuarios WHERE cve=1"));
	...
	if($row=mysql_fetch_array($res)){
		...
		elseif($diferente_pago){
			...
			$res=mysql_query("SELECT a.tipo_pago,a.tipo_venta,a.fecha,a.factura,a.estatus,a.cve,a.plaza,(a.monto+IFNULL(f.recuperacion,0)-IFNULL(e.devolucion,0)) as monto,a.placa,b.nombre as engomado,c.nombre as nomplaza, a.notacredito, a.vale_pago_anticipado, d.localidad_id, c.bloqueada_sat, IFNULL(g.cve,0) as cveentrega  
			FROM cobro_engomado a 
			LEFT JOIN engomados b ON b.cve = a.engomado 
			LEFT JOIN certificados g ON a.plaza = g.plaza AND a.cve = g.ticket AND g.estatus!='C' 
			INNER JOIN plazas c ON c.cve = a.plaza 
			INNER JOIN datosempresas d ON c.cve = d.plaza
			LEFT JOIN 
				(SELECT ticket, SUM(devolucion) as devolucion FROM devolucion_certificado WHERE plaza = '".$row['plaza']."' AND estatus != 'C' AND ticket='".$row['ticket']."') e ON a.cve = e.ticket
			...
			elseif($row['cveentrega']==0 && $Root['facturarsinentrega']==0){
				$resultado['error'] = 1;
				$resultado['mensaje'] .= "No se pueden facturar tickets al cual no se ha entregado su certificado\n";
			}
			elseif($row['factura']>0 && $row['notacredito']==0){
