...
function obtener_informacion($datos){
	...
	$res = mysql_query("SELECT a.fecha, a.motivo, a.cargo, a.abono, a.concepto FROM 
		(SELECT CONCAT(a.fecha, ' ', a.hora) as fecha, CONCAT('Recibo de Salida ', a.cve,', Motivo: ', b.nombre) as motivo, a.monto as cargo, 0 as abono, a.concepto22 as obs FROM recibos_salida a INNER JOIN motivos b ON b.cve = a.motivo WHERE a.plaza='{$datos['cveplaza']}' AND a.estatus != 'C' AND a.fecha BETWEEN '{$datos['busquedafechaini']}' AND '{$datos['busquedafechafin']}'
		 UNION ALL 
