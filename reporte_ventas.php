...
function obtener_informacion($datos){
	...
	elseif ($datos['busquedacertificado'] != ''){
		$select .= " AND b.certificado='{$datos['busquedacertificado']}'";
	}
	elseif ($datos['busquedaplaca'] != ''){
		$select .= " AND a.placa='{$datos['busquedaplaca']}'";
	}
	else{
		
		if ($datos['busquedatipocertificado'] != ''){
			$select .= " AND i.cve='{$datos['busquedatipocertificado']}'";
		}
		...
		if ($datos['busquedatipo_pago']!="") { 
			$select.=" AND a.tipo_pago='{$datos['busquedatipo_pago']}' "; 
		}
...
$res1 = mysql_query("SELECT cve, nombre FROM cat_entidades ORDER BY nombre");
	...
	while($row1=mysql_fetch_array($res1)){
		$array_tipo_prueba[$row1['cve']]=$row1['nombre'];
	}
	...
	<div class="col-xl-12 col-lg-12 col-md-12">
        ...
        <div class="form-group row">
			...
			<div class="col-sm-4">
				...
				$res1 = mysql_query("SELECT a.cve, a.nombre FROM engomados a inner join engomados_plazas b on a.cve = b.engomado WHERE b.plaza = '{$_POST['cveplaza']}' and a.cve in(1,2,3,5,19) ORDER BY a.nombre");
				while($row1=mysql_fetch_array($res1)){
		...
		<div class="form-group row">
			...
			<div class="col-sm-4">
            	<select name="busquedaestatus" id="busquedaestatus" class="form-control"><option value="">Todos</option>
            		...
            		<option value="D">Devuletos</option>
            	</select>
        	</div>
        </div>
        <div class="form-group row">
			...
			<div class="col-sm-4">
            	<input type="number" class="form-control" id="busquedacertificado" name="busquedacertificado" placeholder="Certificado" value="">
        	</div>
        </div>
        <div class="form-group row">
...
?>
	<table class="table">
	  <thead>
	    <tr>
		  ...
		  <th scope="col" style="text-align: center;">Folio Multa</th> 
	...
	?>
	    <tr>
		  ...
		  <td align="center"><?php echo ($row['folio_multa']);?></td>
...
if($_POST['cmd']==100){
	...
	$F->setCellValue('F'.$Line, 'Folio Multa'); 
	$F->setCellValue('G'.$Line, 'Tipo Certificado'); 
	$F->setCellValue('H'.$Line, 'Tipo Venta');
	$F->setCellValue('I'.$Line, 'Tipo de Prueba');
	$F->setCellValue('j'.$Line, 'Voluntario');
	$F->setCellValue('K'.$Line, 'Entidad');
	
	$F->setCellValue('L'.$Line, 'Monto'); 
	$F->setCellValue('M'.$Line, 'Copias'); 
	$F->setCellValue('N'.$Line, 'Total'); 
	$F->setCellValue('O'.$Line, 'Año Certificacion'); 
	$F->setCellValue('P'.$Line, 'Tipo Pago'); 
	$F->setCellValue('Q'.$Line, 'Tipo Vale'); 
	$F->setCellValue('R'.$Line, 'Vale'); 
	$F->setCellValue('S'.$Line, 'Depositante'); 
	$F->setCellValue('T'.$Line, 'Certificado Anterior'); 
	$F->setCellValue('U'.$Line, 'Estatus'); 
	$F->setCellValue('V'.$Line, 'Factura'); 
	$F->setCellValue('W'.$Line, 'Entrega Certificado'); 
	$F->setCellValue('X'.$Line, 'Holograma Entregado'); 
	$F->setCellValue('Y'.$Line, 'Tipo Verificacion Entregada'); 
	$F->setCellValue('Z'.$Line, 'Tecnico'); 
	$F->setCellValue('AA'.$Line, 'Linea'); 
	$F->setCellValue('AB'.$Line, 'Modelo'); 
	$F->setCellValue('AC'.$Line, 'Marca'); 
	$F->setCellValue('AD'.$Line, 'Tipo de Combustible'); 
	$F->setCellValue('AE'.$Line, 'Usuario'); 
	$F->setCellValue('AF'.$Line, 'Motivo Cancelacion'); 
	$F->setCellValue('AG'.$Line, 'Motivo Intento'); 
	$F->setCellValue('AH'.$Line, 'Observaciones'); 
	$totales = array();
	...
	while($row=mysql_fetch_assoc($res)){//extract each record 
		...
		$F->setCellValue('F'.$Line, $row['folio_multa']); 
		$F->setCellValue('G'.$Line, $row['nomengomado']); 
		$F->setCellValue('H'.$Line, $row['nomtipoventa']); 
		$F->setCellValue('I'.$Line, utf8_encode($array_tipo_prueba[$row['tipo_prueba']]));
		$F->setCellValue('J'.$Line, utf8_encode($array_nosi[$row['voluntario']])); 
		$F->setCellValue('K'.$Line, $array_entidad[$row['entidad']]); 
		
		$F->setCellValue('L'.$Line, number_format($row['monto'],2)); 
		$F->setCellValue('M'.$Line, number_format($row['copias'],2)); 
		$F->setCellValue('N'.$Line, number_format($row['monto']+$row['copias'],2)); 
		$F->setCellValue('O'.$Line, $row['nomanio']); 
		$F->setCellValue('P'.$Line, $row['nomtipopago']); 
		$F->setCellValue('Q'.$Line, $row['tipo_vale']); 
		$F->setCellValue('R'.$Line, $row['vale']); 
		$F->setCellValue('S'.$Line, utf8_encode($row['nomdepositante'])); 
		$F->setCellValue('T'.$Line, $row['certificado_anterior']); 
		$F->setCellValue('U'.$Line, utf8_encode($row['nomestatus'])); 
		$F->setCellValue('V'.$Line, $row['factura']); 
		$F->setCellValue('W'.$Line, $row['entrega']); 
		$F->setCellValue('X'.$Line, $row['holograma']); 
		$F->setCellValue('Y'.$Line, utf8_encode($row['nomengomadoentregado'])); 
		$F->setCellValue('Z'.$Line, utf8_encode($row['nomtecnico'])); 
		$F->setCellValue('AA'.$Line, utf8_encode($row['nomlinea'])); 
		$F->setCellValue('AB'.$Line, utf8_encode($row['nommodelo'])); 
		$F->setCellValue('AC'.$Line, utf8_encode($row['nommarca'])); 
		$F->setCellValue('AD'.$Line, utf8_encode($row['nomcombustible'])); 
		$F->setCellValue('AE'.$Line, utf8_encode($row['usuario'])); 
		$F->setCellValue('AF'.$Line, utf8_encode($row['obscan'])); 
		$F->setCellValue('AG'.$Line, utf8_encode($row['nomintento'])); 
		$F->setCellValue('AH'.$Line, utf8_encode($row['obs'])); 
		$i++;
		$totales[0]+=$row['monto'];
		$totales[1]+=$row['copias'];
		$totales[2]+=$row['monto']+$row['copias'];
	...
	$F->setCellValue('K'.$Line, 'TOTAL:'); 
	$F->setCellValue('L'.$Line, number_format($totales[0],2)); 
	$F->setCellValue('M'.$Line, number_format($totales[1],2)); 
	$F->setCellValue('N'.$Line, number_format($totales[2],2)); 
