...
function obtener_informacion($datos){
	...
	if($datos['mostrar']<2){
		...
		$select .= " JOIN cobro_engomado d ON a.plaza = d.plaza AND ((a.cve = d.vale_pago_anticipado AND d.tipo_venta=0 AND d.tipo_pago=6) OR (a.cve=d.codigo_cortesia AND d.tipo_venta=2 AND d.tipo_cortesia=2)) AND d.estatus!='C'";
	}
