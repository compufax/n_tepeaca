...
function obtener_informacion($datos){
	...
	$res = mysql_query("SELECT plaza, SUM(copias*costo_copias) FROM cobro_engomado WHERE estatus!='C' AND tipo_pago NOT IN (2,6, 12) AND fecha BETWEEN '{$datos['busquedafechaini']}' AND '{$datos['busquedafechafin']}' GROUP BY plaza");
	while($row = mysql_fetch_array($res)){
