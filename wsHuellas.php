...
function ConsultarPersonas($usuario, $password, $id, $empresa = 4)
{
	...
	{
		if ($id <= 0){
			$respuesta['mensaje']='Necesita ingresar la credencial';
		}
		else{
...
function RegistraChecada($usuario, $password, $lector, $operador, $tipo = 0)
{
	...
	if($respuesta['mensaje']=='')
	{
		/*if($tipo==2 && date('H:i:s')>'16:00:00'){
			...
			$limite=date( "Y-m-d H:i:s" , strtotime ( "-5 minute" , strtotime(date("Y-m-d H:i:s")) ) );
			$res = mysql_query("SELECT  a.fechahora FROM checada_lector a inner join motivos_checada b on a.tipo = b.cve WHERE a.cvepersonal='$operador' AND DATE(a.fechahora)='".date("Y-m-d")."' AND a.tipo={$tipo} ORDER BY fechahora DESC");
			$checadas = mysql_num_rows($res);
			if($checadas>=1){
				if($tipo==1)
					$respuesta['mensaje']='Ya checo entrada a trabajar ';
				elseif($tipo==4)
					$respuesta['mensaje']='Ya checo salida a comer ';
				elseif($tipo==3)
					$respuesta['mensaje']='Ya checo entrada de comer ';
				elseif($tipo==2)
					$respuesta['mensaje']='Ya checo salida de trabajar ';
			}
			else{
