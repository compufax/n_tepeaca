...
function obtener_informacion($datos){
	...
	while($row = mysql_fetch_assoc($res)){
		...
		$res1=mysql_query("SELECT tipo, RIGHT(MIN(fechahora), 8) as hora, TIME_TO_SEC(RIGHT(MIN(fechahora), 8)) as segundos  FROM checada_lector WHERE cvepersonal = '{$row['cve']}' AND DATE(fechahora)=CURDATE() GROUP BY tipo ORDER BY fechahora");
		while($row1 = mysql_fetch_array($res1)){
			$renglon['hora_'.$row1['tipo']] = $row1['hora'];
		}
		$resultado[] = $renglon;
	}
...
?>
	<table class="table">
	  <thead>
	    <tr>
	      ...
	      <th scope="col" style="text-align: center;">Hora Entrada Trabajo</th>  
	      <th scope="col" style="text-align: center;">Hora Salida Comer</th> 
	      <th scope="col" style="text-align: center;">Hora Entrada Comer</th> 
	      <th scope="col" style="text-align: center;">Hora Salida Trabajo</th> 
	    </tr>
	  </thead>
	  <tbody>
	<?php
	...
	?>
	    <tr>
	      ...
	      <td align="center"><?php echo $row['hora_1'];?></td>
	      <td align="center"><?php echo $row['hora_4'];?></td>
	      <td align="center"><?php echo $row['hora_3'];?></td>
	      <td align="center"><?php echo $row['hora_2'];?></td>
	    </tr>
	<?php
		}
	?>
