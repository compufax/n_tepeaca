...
function generaFacturaPdf($plaza,$cvefact,$mostrar=0,$tipodocumento=1){
	...
	if($row['respuesta1']!=''){
		...
		$row2['regimensat']= $arreglo['cfdi:Comprobante'][0]['cfdi:Receptor'][0]['@RegimenFiscalReceptor'];
		$row2['codigopostal']= $arreglo['cfdi:Comprobante'][0]['cfdi:Receptor'][0]['@DomicilioFiscalReceptor'];
	}
	...
	if($row['estatus']=='C'){
		if(file_exists("img/cancelado.jpg")) $pdf->Image("img/cancelado.jpg",10,45,190,200);
	}
...
MEXICO');
	...
	$pdf->Cell(15,4,"Placa",1,0,"C",0);
	$pdf->Cell(10,4,"Cant.",1,0,"C",0);
	$pdf->Cell(15,4,"Unidad",1,0,"C",0);
	$pdf->Cell(65,4,"Concepto / Descripción",1,0,"C",0);
	...
	while($row2 = mysql_fetch_array($res2)){
		...
		$pdf->Cell(15,4,$row3['placa'],1,0,"R",0);
		$pdf->Cell(10,4,$row2['cantidad'],1,0,"R",0);
		$pdf->Cell(15,4,$row2['unidad'],1,0,"C",0);
		$y3=$pdf->GetY();
		if($y!=$y3) $y=$y3;
		$pdf->MultiCell(65,4,$row2['concepto'],1,"J",0);
		$y2=$pdf->GetY();
		$pdf->SetXY(125,$y);
		$pdf->Cell(15,4,round($row2['importe']/$row2['cantidad'],6),1,0,"R",0);
...
function generaFacturaPdfXml($cvefact,$mostrar=0){
	global $base,$array_tipo_pago,$array_forma_pago;

	$res = mysql_query("SELECT * FROM xml_sat WHERE cve='".$cvefact."'");
	$row = mysql_fetch_array($res);
	...
	$nombre='FACTURA';
	$tipocomprobante = 'INGRESO';
	if($row['tipo_comprobante'] == 'E'){
		$nombre='NOTA DE CREDITO';
		$tipocomprobante = 'EGRESO';
	}
	$arch = 'cfdi/comprobantes/cfdix_'.$row['plaza'].'_'.$cvefact.'.xml';
	$cadena= file_get_contents($arch);
	$dom = new DOMDocument;
	$dom->loadXML($cadena);
	$arreglo = _xmlToArray($dom);
	...
	$pdf->SetXY(65,10);
	$pdf->MultiCell(80,5,$row['nombre_emisor'].'
'.$row['rfc_emisor'].'
...
MEXICO');
	...
	$pdf->SetX(170);
	$pdf->Cell(20,4,'HORA:  ',0,0,'R');
	$pdf->Cell(20,4,substr($row['fecha'],11),0,0,'L');
	$pdf->Ln();
	if($y<$pdf->GetY()){
		$y = $pdf->GetY();
	}
	...
	$pdf->Cell(20,4,'Cliente:');
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(70,4,$row['nombre_receptor']);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(20,4,'R.F.C.:');
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(70,4,$row['rfc_receptor']);
	$rr=$row['rfc_receptor'];
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(20,4,'CP:');
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(70,4,$row['direccion_receptor']);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(20,4,'USO CFDI:');
	$pdf->SetFont('Arial','',8);
	$pdf->Cell(20,4,$row['uso_cfdi'].' '.$array_usocfdi[$row['uso_cfdi']]);
	$pdf->Ln();
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(20,4,'REGIMEN:');
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(80,4,$row['regimen_receptor'].' '.$array_regimensat[$row['regimen_receptor']]);
	$pdf->Ln();
	$pdf->Ln();
	$y2=$pdf->GetY();
	...
	$pdf->Cell(20,4,'REGIMEN:');
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(80,4,$row['regimen_emisor'].' '.$array_regimensat[$row['regimen_emisor']]);
	$pdf->Ln();
	$pdf->SetX(110);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(35,4,'LUGAR DE EXPEDICION:');
	...
	$pdf->Ln();
	$pdf->SetX(110);
	$pdf->MultiCell(90,4,$row['lugar_expedicion']);
	...
	$pdf->Cell(50,4,'RFC PROVEEDOR CERTIFICADO:');
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(90,4,$row['rfc_prov_certif']);
	$pdf->Ln();
	if($y2<$pdf->GetY()){
		$y2 = $pdf->GetY();
	}
	...
	$pdf->Cell(10,4,"Cant.",1,0,"C",0);
	$pdf->Cell(15,4,"Unidad",1,0,"C",0);
	$pdf->Cell(90,4,"Concepto / Descripción",1,0,"C",0);
	$pdf->Cell(15,4,"Valor Unit",1,0,"C",0);
	$pdf->Cell(15,4,"Tasa",1,0,"C",0);
	$pdf->Cell(15,4,"Factor",1,0,"C",0);
	$pdf->Cell(22.5,4,"Impuestos",1,0,"C",0);
	...
	$y=$pdf->GetY();
	$pdf->SetFont('Arial','',8);
	$res2 = mysql_query("SELECT * FROM satmov_xml WHERE plaza={$plaza} AND cvefact={$cvefact}");
	while($row2 = mysql_fetch_array($res2)){
		$pdf->SetXY(10,$y);
		$pdf->Cell(10,4,$row2['cantidad'],1,0,"R",0);
		$pdf->Cell(15,4,$row2['unidad'],1,0,"C",0);
		$y3=$pdf->GetY();
		if($y!=$y3) $y=$y3;
		$pdf->MultiCell(90,4,$row2['descripcion'],1,"J",0);
		$y2=$pdf->GetY();
		$pdf->SetXY(125,$y);
		$pdf->Cell(15,4,$row['valor_unitario'],1,0,"R",0);
		$pdf->Cell(15,4,0.1600,1,0,"R",0);
		$pdf->Cell(15,4,'Tasa',1,0,"C",0);
		$pdf->Cell(12.5,4,'002 IVA',1,0,"C",0);
		$row3 = mysql_fetch_assoc(mysql_query("SELECT importe FROM satmovimpuestos_xml WHERE plaza={$plaza} AND cvefact={$cvefact} AND cvefactmov = {$row2['cve']} AND tipo=1 AND impuesto='002'"));
		$pdf->Cell(10,4,$row3['importe'],1,0,"R",0);
		$pdf->Cell(15,4,$row2['importe'],1,0,"R",0);
		$y=$y2;
	}
	$pdf->SetXY(10,$y);
	...
	if($row['metodo_pago'] == 'PUE'){
		$pdf->MultiCell(157,4,numlet($row['total']).'
Metodo pago: PUE PAGO EN UNA SOLA EXHIBICION
Forma pago: '.$array_tipo_pago[$row['forma_pago']].'
Condiciones: CONTADO
MONEDA: MXN');
	}
	else{
...
MONEDA: MXN');		
	...
	$pdf->SetXY(10,$y2);
	$tt=number_format($row['total'],6,".","");
	QRcode::png("?re=".$re."&rr=".$rr."&tt=".$tt."&id=".$row['uuid'],"cfdi/comprobantes/barcodex_{$row['plaza']}_{$row['cve']}.png","L",4,0);
	if(file_exists("cfdi/comprobantes/barcodex_{$row['plaza']}_{$row['cve']}.png")) $pdf->Image("cfdi/comprobantes/barcodex_{$row['plaza']}_{$row['cve']}.png",20,$y2,34,34);
	$pdf->SetXY(60,$y2);
	...
	$pdf->Ln();
	$pdf->SetX(80);
	$pdf->Cell(100,4,'SERIE DEL SELLO: '.$row['no_certificado']);
	$pdf->Ln();
	$pdf->Cell(70,4,'No de Serie del Certificado del SAT:          ',0,0,'R');
	$pdf->Cell(100,4,$row['no_certificado_sat']);
	$pdf->Ln();
	$pdf->Cell(70,4,'Fecha y hora de certificación:          ',0,0,'R');
	$pdf->Cell(100,4,date('j/n/Y - H:i:s', strtotime($row['fechatimbre'])));
	$pdf->Ln();
	...
	$pdf->SetX(20);
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(180,3,$row['sello'],0,"C",0);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(162.5,4,"Sello del SAT",0,0,"L");
	$pdf->Ln();
	$pdf->SetX(20);
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(180,3,$row['sello_sat'],0,"C",0);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(162.5,4,"Cadena original del complemento de certificación digital del SAT",0,0,"L");
	$pdf->Ln();
	$pdf->SetX(20);
	$pdf->SetFont('Arial','',8);
	$cadenaoriginal = '||'.$row['vesrion'].'|'.$row['uuid'].'|'.str_replace(' ','T',$row['fecha_timbrado']).'|'.$row['rfc_prov_certif'].'|'.$row['sello_cfd'].'|'.$row['no_certificado_sat'].'||';
	$pdf->MultiCell(180,3,$cadenaoriginal,0,"C",0);
	...
	if($mostrar==1){
		...
		$pdf->Output("cfdi/comprobantes/cfdix_{$row['plaza']}_{$row['cve']}.pdf","F");
	}
	if(file_exists("cfdi/comprobantes/barcodex_{$row['plaza']}_{$row['cve']}.png")) unlink("cfdi/comprobantes/barcodex_{$row['plaza']}_{$row['cve']}.png");
}
?>