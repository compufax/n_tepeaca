...
function obtener_informacion($datos){
	$resultado = array();
	$res = mysql_query("SELECT b.cve, b.nombre, a.importe FROM (SELECT motivo, SUM(monto) as importe FROM recibos_salida WHERE plaza='{$datos['cveplaza']}' AND estatus!='C' AND fecha BETwEEN '{$datos['busquedafechaini']}' AND '{$datos['busquedafechafin']}' GROUP BY motivo) a INNER JOIN motivos b ON b.cve = a.motivo ORDER BY b.nombre");
...
<h1>Detalle <?php echo $Motivo['nombre'];?> del <?php echo mostrar_fechas($_POST['busquedafechaini']); ?> al <?php echo mostrar_fechas($_POST['busquedafechafin']);?></h1>
<div class="row" id="resultadocorte">
	<table class="table">
	  <thead>
	    <tr>
