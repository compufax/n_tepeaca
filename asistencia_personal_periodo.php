...
$domingos_asistidos = array('2023-06-25', '2023-07-30', '2023-08-06');

function obtener_informacion($datos){
	global $domingos_asistidos;
	...
	while($row = mysql_fetch_assoc($res)){
		...
		while($fecha<=$datos['busquedafechafin']){
			...
			if($dia != 0 || in_array($fecha, $domingos_asistidos)){
				$res1=mysql_query("SELECT RIGHT(a.fechahora, 8) as hora, TIME_TO_SEC(RIGHT(a.fechahora, 8)) as segundos, b.plaza  FROM checada_lector a LEFT JOIN series b ON b.cve = a.cvelector WHERE a.cvepersonal = '{$row['cve']}' AND a.fechahora BETWEEN '{$fecha} 00:00:00' AND '{$fecha} 23:59:59' AND a.tipo=1 ORDER BY a.fechahora");
				...
				$res1=mysql_query("SELECT RIGHT(a.fechahora, 8) as hora, TIME_TO_SEC(RIGHT(a.fechahora, 8)) as segundos, b.plaza  FROM checada_lector a LEFT JOIN series b ON b.cve = a.cvelector WHERE a.cvepersonal = '{$row['cve']}' AND a.fechahora BETWEEN '{$fecha} 00:00:00' AND '{$fecha} 23:59:59' AND a.tipo=4 ORDER BY a.fechahora");
				$row1 = mysql_fetch_array($res1);
				$hora_salida_comer = $row1['hora'];
				if($row1['plaza'] != $AsistenciaPersonal['plaza']){
					$hora_salida_comer = '<font color="RED">'.$row1['hora'].'</font>';
				}
				$segundos_salida_comer = $row1['segundos'];

				$res1=mysql_query("SELECT RIGHT(a.fechahora, 8) as hora, TIME_TO_SEC(RIGHT(a.fechahora, 8)) as segundos, b.plaza  FROM checada_lector a LEFT JOIN series b ON b.cve = a.cvelector WHERE a.cvepersonal = '{$row['cve']}' AND a.fechahora BETWEEN '{$fecha} 00:00:00' AND '{$fecha} 23:59:59' AND a.tipo=3 ORDER BY a.fechahora");
				$row1 = mysql_fetch_array($res1);
				$hora_entrada_comer = $row1['hora'];
				if($row1['plaza'] != $AsistenciaPersonal['plaza']){
					$hora_entrada_comer = '<font color="RED">'.$row1['hora'].'</font>';
				}
				$segundos_entrada_comer = $row1['segundos'];

				$res1=mysql_query("SELECT RIGHT(a.fechahora, 8) as hora, TIME_TO_SEC(RIGHT(a.fechahora, 8)) as segundos, b.plaza  FROM checada_lector a LEFT JOIN series b ON b.cve = a.cvelector WHERE a.cvepersonal = '{$row['cve']}' AND a.fechahora BETWEEN '{$fecha} 00:00:00' AND '{$fecha} 23:59:59' AND a.tipo=2 ORDER BY a.fechahora");
				$row1 = mysql_fetch_array($res1);
				...
				$renglon['fechas'][$fecha]['hora_salida_comer'] = $hora_salida_comer;
				$renglon['fechas'][$fecha]['hora_entrada_comer'] = $hora_entrada_comer;
				$renglon['fechas'][$fecha]['hora_salida'] = $hora_salida_trabajo;
				$tiempo_trabajo=0;
				$tiempo_comer=0;
				if($segundos_entrada_trabajo > 0 && $segundos_salida_trabajo > 0){
					$tiempo_trabajo += $segundos_salida_trabajo - $segundos_entrada_trabajo;
				}
        	...
        	<div class="col-sm-12" align="center">
				...
				<button class="btn btn-info dropdown-toggle" type="button" id="dropdownMenuButton3" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
							...
							<div class="dropdown-menu" aria-labelledby="dropdownMenuButton3">
							   <!-- <a class="dropdown-item" href="#" onClick="atcr('asistencia_personal_periodo.php','_blank', 102, 0);">Termico</a>-->
							    <a class="dropdown-item" href="#" onClick="atcr('asistencia_personal_periodo.php','_blank', 100, 1);">Excel</a>
							</div>
...
header("Content-Disposition: attachment; filename=asistencia_periodo.xls");
header("Pragma: no-cache");
header("Expires: 0");


echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="" id="tabla1" >';
	echo '<!--<tr style="font-size:26px"><td align="center">'.$array_plaza[$_POST['plazausuario']].'</td></tr>--><tr style="font-size:24px">
			<td align="center" colspan="8" style="font-size:24px">Reporte de Nominas por Grupo</td>
		 </tr>';
	echo '</table>';
	echo '<br>';
	    ...
	    <tr>
	      ...
	      <?php
			...
			while($fecha<=$_POST['busquedafechafin']){
				...
				$dia=date("w", mktime(0, 0, 0, intval($arfecha[1]), intval($arfecha[2]), $arfecha[0]));
				if($dia != 0 || in_array($fecha, $domingos_asistidos)){
					echo '<th scope="col" colspan="5" style="text-align: center; border-left: 2px solid #000000;">'.$fecha.'</th> ';
				}
				$fecha=date( "Y-m-d" , strtotime ( "+1 day" , strtotime($fecha) ) );
			}
		  ?>
	      ...
	      <?php
			...
			while($fecha<=$_POST['busquedafechafin']){
				...
				if($dia != 0 || in_array($fecha, $domingos_asistidos)){
					...
					echo '<th scope="col" style="text-align: center;">Hora Salida Comer</th>';
					echo '<th scope="col" style="text-align: center;">Hora Entrada Comer</th>';
	      ...
	      <?php
			...
			while($fecha<=$_POST['busquedafechafin']){
				...
				if($dia != 0 || in_array($fecha, $domingos_asistidos)){
					...
					echo '<td align="center">'.$row['fechas'][$fecha]['hora_salida_comer'].'</td>';
					echo '<td align="center">'.$row['fechas'][$fecha]['hora_entrada_comer'].'</td>';
