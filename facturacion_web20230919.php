...
if($_POST['ajax']==1){
	...
	if($row=mysql_fetch_array($res)){
		...
		elseif($diferente_pago){
			...
			/*elseif($row['fecha']<(date('Y-m').'-01')){//if($row['fecha']<=$limite2 || ($row['fecha']<=$limite1 && date('d')>'07')){
				$resultado['error'] = 1;
				$resultado['mensaje'] .= 'Vencio el limite de facturación'."\n";
			}*/
...
if($_POST['ajax']=='prefactura'){
	...
	if($row = mysql_fetch_array($res)){
		echo '<h1>El sat nos pide validar su rfc.<br>
Nos puede mandar foto del ticket y datos fiscales por favor</h1>';
...
if($_POST['cmd']==2){
	...
	if($row=mysql_fetch_array($res)){
		...
		$copias=0;
		$monto_copias=0;
		$res=mysql_query("SELECT a.cve, (a.monto+IFNULL(f.recuperacion,0)-IFNULL(e.devolucion,0)) as monto, a.copias, (a.copias*a.costo_copias) as monto_copias, a.fecha, a.placa FROM cobro_engomado a 
			LEFT JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket AND b.estatus!='C' 
			LEFT JOIN 
				(SELECT ticket, SUM(devolucion) as devolucion FROM devolucion_certificado WHERE plaza = '".$plaza."' AND estatus != 'C' AND ticket IN (".$tickets.") GROUP BY ticket) e ON a.cve = e.ticket
			LEFT JOIN 
		...
		if(count($array_detalles)>0){
			...
			if($copias>0){
				$monto = $monto_copias;
				$importe_iva=round($monto*16/116,2);
				$total+=round($monto,2);
				$subtotal+=round($monto-$importe_iva,2);
				$iva+=$importe_iva;
				$concepto = 'COPIAS';
				...
				precio='".round(round($monto-$importe_iva,2)/$copias,2)."',importe='".round($monto-$importe_iva,2)."',
			...
			if(trim($_POST['rfc'])!='RAGS660522NT3'){
				mysql_query("UPDATE facturas SET estatus='P' WHERE plaza={$plaza} AND cve={$cvefact}");
					...
					if ($oSoapClient->fault) {
						...
						if ($err){
							...
							if($respuesta['resultado']){
								...
								echo '<h1>Se genero el folio de factura '.$fserie." ".$ffolio.'  <a href="#" onClick="atcr(\'facturacion_web.php\',\'_blank\',\'200\',\''.$plaza.'|'.$cvefact.'|1\');"><img src="img/zip_grande.png" border="0" width="45px" height="45px" title="Descargar">Descarga aqui su factura</a></h1>';
							}
							else
								$strmsg=$respuesta['mensaje'];
							//print_r($respuesta);	
				...
				mysql_query("UPDATE facturas SET estatus='A' WHERE plaza={$plaza} AND cve={$cvefact}");
			}
			}
		}
	}
...
<div id="dialog_confirmacion" style="display:none;">
	<h1><font color="RED">¿Los datos son correctos?. No habrá cancelaciones</font></h1>
</div>
	...
	<div class="row">
		...
		<div class="col-sm-6">
			<div class="form-group">
				...
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="rfc" name="rfc" maxlength="13" onKeyUp="this.value=this.value.toUpperCase();" onChange="traerDatosCli(this.value)" value="<?php echo $row['rfc'];?>" <?php echo $readOnly; ?>>
			...
			<div class="form-group">
				...
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="nombre" name="nombre" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['nombre'];?>" <?php echo $readOnly; ?>>
				</div>
			</div>

			<div class="form-group">
				<label for="nombre" class="col-sm-5 control-label">R&eacutegimen Fiscal</label>
			...
			<div class="form-group ctipospago" style="display:none;">
				<label for="rfc" class="col-sm-5 control-label">Tipo de Pago</label>
				<div class="col-sm-7">
					<select name="tipo_pago" id="tipo_pago" onChange="
			...
			<div class="form-group ccuenta" style="display:none;">
				<label for="rfc" class="col-sm-5 control-label">Cuenta</label>
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="cuenta" name="cuenta" maxlength="4" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row1['cuenta'];?>" <?php echo $readOnly; ?>>
		...
		<div class="col-sm-6">
			<div class="form-group" style="display:none;">
				...
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="calle" name="calle" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['calle'];?>" <?php echo $readOnly; ?>>
				</div>
			</div>
			...
			<div class="form-group" style="display:none;">
				...
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="numexterior" name="numexterior" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['numexterior'];?>" <?php echo $readOnly; ?>>
				</div>
			</div>
			...
			<div class="form-group" style="display:none;">
				...
				<div class="col-sm-7">
					<input type="text" class="mayusculas form-control input-sm" id="numinterior" name="numeinterior" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['numinterior'];?>" <?php echo $readOnly; ?>>
				</div>
			</div>
			...
			<div class="form-group" style="display:none;">
				...
				<div class="col-sm-7">
					<input type="text" class="mayusculas form-control input-sm" id="colonia" name="colonia" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['colonia'];?>" <?php echo $readOnly; ?>>
				</div>
			</div>
			...
			<div class="form-group" style="display:none;">
				...
				<div class="col-sm-7">
					<input type="text" class="mayusculas form-control input-sm" id="localidad" name="localidad" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['localidad'];?>" <?php echo $readOnly; ?>>
			...
			<div class="form-group" style="display:none;">
				...
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="municipio" name="municipio" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['municipio'];?>" <?php echo $readOnly; ?>>
				</div>
			</div>
			...
			<div class="form-group" style="display:none;">
				...
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="estado" name="estado" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['estado'];?>" <?php echo $readOnly; ?>>
					<span class="text-muted"><small>Esta prohibido usar Distrito Federal</small></span>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="codigopostal" name="codigopostal" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['codigopostal'];?>" <?php echo $readOnly; ?>>
				</div>
			</div>
